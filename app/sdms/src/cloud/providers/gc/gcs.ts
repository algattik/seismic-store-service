...
const GCS_URL_SEPARATOR = "://";

export class SubProjectPath {
    public dataPartitionId?: string
    public subprojectFolder?: string
    public bucketname: string // TODO: delete this, as it is old implementation with creating a new bucket for a new subproject
     ...
     * The subproject path could point either to the separate google storage bucket or to the separate folder in the bucket got form Partition service by dataPartitionId
     ...
     * @returns 
     */
    public static async create(gcs_path: string): Promise<SubProjectPath> {
        if (!gcs_path.includes(GCS_URL_SEPARATOR)) {
            const bucketname = gcs_path;
            return new SubProjectPath(bucketname, "", "");
        } else {
            const splitted_bucket_name = gcs_path.split(GCS_URL_SEPARATOR);
            const dataPartitionId = splitted_bucket_name[0];
            const subprojectFolder = splitted_bucket_name[1];
            const bucketname = await SubProjectPath.getBucketNameFromPartitionService(dataPartitionId);
            return new SubProjectPath(bucketname, subprojectFolder, dataPartitionId);
        }
    }

    private static async getBucketNameFromPartitionService(dataPartitionId: string) {
        // TODO: implement Partition Service call
        return ConfigGoogle.GSC_BUCKET
    }
}
...
export class GCS extends AbstractStorage {
    ...
    public constructor(tenant: TenantModel) {
        ...
        this.gcsBucket = ConfigGoogle.GSC_BUCKET;
    }

    private async getSubprojectPath(gcs_path: string): Promise<SubProjectPath>  {
        const subprojectPath = await SubProjectPath.create(gcs_path);
    ...
    public async randomBucketName(): Promise<string> {
        ...
        return this.dataPartitionId + GCS_URL_SEPARATOR + randomFolderName;
    }
    ...
    public async createBucket(subprojectURI: string, location: string, storageClass: string): Promise<void> {
        ...
        bucket.file(newSubprojectFolderDummyObj).save("");
    }
    ...
    public async deleteObjects(subprojectURI: string, datasetFolder: string): Promise<void> {
        ...
        bucket.deleteFiles(deleteQuery)
            .catch(
    ...
    public async copy(
        ...
        do {
            ...
            for (const file of files[0]) {
                ...
                fileNameTo = fileNameTo + (rmDatasetFolderFrom ? file.name.substr(datasetFolderFrom.length) : file.name);
    ...
    public async bucketExists(subprojectURI: string): Promise<boolean> {
        ...
        const bucket = this.getStorageClient().bucket(subprojectPath.bucketname);
        try {
            const does_exist = await bucket.file(
